...
def marcar_estado_cuenta(file_bytes: bytes, filas_usadas: set, banco: str = "") -> bytes:
    """
    Carga el estado de cuenta y marca las filas incluidas en la póliza
    con color de banco específico y fuente negrita profesional.
    BBVA=azul · BANORTE=rojo · INBURSA=verde.
    """
    BANK_STYLE = {
        "BBVA":    ("A9C9EF", "004481"),   # (fondo, color_texto)
...
def generar_excel(registros: list, plantilla=None) -> bytes:
    ...
    # ── Paleta suave profesional ──
    F_ADMIN   = PatternFill("solid", fgColor="4B5563")   # gris pizarra suave
    F_H_BBVA  = PatternFill("solid", fgColor="2563EB")   # azul medio BBVA
    F_H_BNT   = PatternFill("solid", fgColor="DC2626")   # rojo medio Banorte
    F_H_INB   = PatternFill("solid", fgColor="059669")   # verde medio Inbursa
    F_H_TCARG = PatternFill("solid", fgColor="D97706")   # ámbar — TOTAL CARGOS
    F_H_AMEX  = PatternFill("solid", fgColor="4F46E5")   # índigo suave AMEX
    F_H_EFEC  = PatternFill("solid", fgColor="E11D48")   # rosa-rojo EFECTIVALE
    F_H_EDEN  = PatternFill("solid", fgColor="EA580C")   # naranja EDENRED
    F_H_CAJA  = PatternFill("solid", fgColor="F59E0B")   # ámbar CAJA (texto oscuro)
    F_H_BNTTR = PatternFill("solid", fgColor="B91C1C")   # rojo medio Banorte transit
    F_H_SHLL  = PatternFill("solid", fgColor="CA8A04")   # dorado Shell (texto oscuro)
    F_H_BBVAT = PatternFill("solid", fgColor="1D4ED8")   # azul medio BBVA transit
    F_H_INBTR = PatternFill("solid", fgColor="047857")   # verde medio Inbursa transit
    F_H_TABON = PatternFill("solid", fgColor="15803D")   # verde total abonos
    F_H_DIFF  = PatternFill("solid", fgColor="7C3AED")   # violeta diferencia
    # Fila 2 — números de cuenta
    F_GRAY2   = PatternFill("solid", fgColor="374151")   # gris oscuro suave
    # Filas de datos — tonos muy claros por banco
    ...
    # Headers con fondo claro requieren texto oscuro
    _DARK_TEXT_COLS = {15, 17}   # CAJA=col15, SHELL=col17

    # ── Bordes ──
    _S = Side(style="thin", color="999999")
    BORDER = Border(left=_S, right=_S, top=_S, bottom=_S)
    _SH = Side(style="medium", color="FFFFFF")
    BORDER_H = Border(left=_SH, right=_SH, top=_SH, bottom=_SH)  # encabezados
    ...
    headers_admin = [
        (1,  "TIPO",       F_ADMIN),
        (2,  "FECHA",      F_ADMIN),
        (3,  "REFERENCIA", F_ADMIN),
        (4,  "CONCEPTO",   F_ADMIN),
        (5,  "ERROR",      F_ADMIN),
        (6,  "UIDD",       F_ADMIN),
        (7,  "NÚM PÓLIZA", F_ADMIN),
        (8,  "PROCESADO",  F_ADMIN),
    ]
    for col, lbl, fill in headers_admin:
        set_cell(ws, 3, col, value=lbl, font=fnt_h(), fill=fill,
                 align=A_CTR, border=BORDER_H)

    # Cols cargo banco — color específico por banco
    ...
    # Cols abono — color individual por tipo de depósito
    ABONO_FILLS = {
        12: F_H_AMEX, 13: F_H_EFEC, 14: F_H_EDEN, 15: F_H_CAJA,
        16: F_H_BNTTR, 17: F_H_SHLL, 18: F_H_BBVAT, 19: F_H_INBTR,
    }
    for a in ABONOS:
        set_cell(ws, 3, a["col"] + 1,
                 value=a["nombre"],
                 font=fnt_h(), fill=ABONO_FILLS.get(a["col"], F_ADMIN),
    ...
    for fila_num, r in enumerate(registros_sorted, start=4):
        ...
        dat(1,  "I", align=A_CTR)  # siempre "I"
        dat(2,  r["fecha"], num_fmt=FMT_DATE)
        dat(3,  r["ref"],   align=A_LEFT)               # "DEPOSITOS BBVA/BANORTE/INBURSA"
    ...
    ws.row_dimensions[3].height = 36   # encabezados más altos y legibles
    ...
    if plantilla is None:
        ...
        for col, lbl, fill in [(1,"N° Cuenta",F_TH_C),(2,"Banco",F_TH_C),
        ...
        for i, (banco, (cuenta, nombre)) in enumerate(CARGOS.items(), start=3):
            ...
            set_cell(wc, i, 1, cuenta,         font=fn_row, fill=fill, align=A_L, border=BORDER)
            set_cell(wc, i, 2, nombre.strip(), font=fn_row, fill=fill, align=A_L, border=BORDER)
        for i, a in enumerate(ABONOS, start=3):
            fill = F_ROW1 if i % 2 == 0 else F_ROW2
            set_cell(wc, i, 4, a["cuenta"], font=fn_row, fill=fill, align=A_L, border=BORDER)
            set_cell(wc, i, 5, a["nombre"], font=fn_row, fill=fill, align=A_L, border=BORDER)
